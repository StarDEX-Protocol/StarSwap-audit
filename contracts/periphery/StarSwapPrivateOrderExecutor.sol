 ...
 *
 */
contract StarSwapPrivateOrderExecutor is
  IStarSwapEvents,
  IStarSwapErrors,
  Context,
  ReentrancyGuard,
  ERC721Holder
{
  ...
  function invalidatePrivateOrder(PrivateOrder memory order) external {
    if (_msgSender() != order.maker) revert StarSwap__NotAnOwner();
  ...
  function fillPrivateOrder(
    ...
    if (privateOrder.recipient != address(0) && privateOrder.recipient != _msgSender()) {
      revert StarSwap__YouAreNotARecipient();
    }
    if (
      ...
      revert StarSwap__OrderHaveAlreadyBeenExecuted();
    }
    ...
    uint256 orderId = starswap.createPublicOrder(order);

    result = starswap.fillPublicOrder(orderId, _msgSender());
