...
import { StarSwap } from '../StarSwap.sol';
import '../OrderStructs.sol';

/**
 * @notice A facade contract for StarSwap that allows for batch execution of public orders.
 */
contract StarSwapBatchExecutor is Context {
  using SafeERC20 for IERC20;
  StarSwap public starswap;

  constructor(StarSwap _starswap) {
    starswap = _starswap;
  }
  ...
  function batchFillPublicOrdersWithEntryPermit(
    ...
    PublicOrder memory order = starswap.orderManager().getOrder(
  ...
  function batchFillPublicOrders(
    ...
    for (uint256 i = 0; i < orderFillRequests.length; i++) {
      ...
      IERC20(order.makerBuyToken).approve(address(starswap), type(uint256).max);

      results[i] = OrderLib.isFillRequestPartial(orderFillRequests[i], order)
        ? starswap.fillPublicOrderPartially(orderFillRequests[i], _msgSender())
        : starswap.fillPublicOrder(orderFillRequests[i].orderId, _msgSender());
    }
  }
  ...
  function batchFillPublicOrdersInSequence(
    ...
    for (uint256 i = 0; i < orderFillRequests.length; i++) {
      ...
      results[i] = OrderLib.isFillRequestPartial(orderFillRequests[i], order)
        ? starswap.fillPublicOrderPartially(orderFillRequests[i], address(this))
        : starswap.fillPublicOrder(orderFillRequests[i].orderId, address(this));
    }
