...
describe('FeeAggregatorPlugin', () => {
  ...
  it('should assign the owner', async () => {
    ...
    const isOwner = await starSwap.hasRole(
  ...
  it('should accrue fees when public market order is executed', async () => {
    ...
    await wmaticToken
      .connect(user2)
      .approve(starSwap.getAddress(), ethers.parseEther('2'));

    await starSwap.connect(user2).fillPublicOrder(0, user2.getAddress());

    expect(await erc20Token.balanceOf(starSwap.getAddress())).to.equal(
  ...
  it('owner should be allowed to withdraw fees', async () => {
    ...
    await starSwap.withdraw(erc20Token.getAddress(), ethers.parseEther('0.0024'));
  ...
  it('should not allow StarSwap admin to withdraw tokens that were deposited by makers when creating orders', async () => {
    const { starSwap, erc20Token, wmaticToken } = await loadFixture(prepareTestEnv);
    ...
    expect(
      starSwap.withdraw(erc20Token.getAddress(), ethers.parseEther('1')),
    ).to.be.revertedWithCustomError(
      starSwap,
      'StarSwap__WithdrawalViolatesFullBackingRequirement',
    );
  });

  it('should allow owner to set a fee for a specific token', async () => {
    const { starSwap, feeAggregatorPlugin, erc20Token, wmaticToken, usdcToken } =
    ...
    await usdcToken.approve(starSwap.getAddress(), ethers.parseUnits('1', 6));
    await starSwap.createPublicOrder({
    ...
    await starSwap.connect(user2).fillPublicOrder(1, user2.getAddress());

    // default fee is 0.24%, if not set for a specific token
    expect(await usdcToken.balanceOf(starSwap.getAddress())).to.equal(
  ...
  it('should allow to disable swap fee by setting the fee to 0', async () => {
    const { starSwap, feeAggregatorPlugin, erc20Token, wmaticToken } =
      await loadFixture(prepareTestEnv);
    const [, user2] = await ethers.getSigners();
    ...
    expect(await erc20Token.balanceOf(starSwap.getAddress())).to.equal(0);
  });
  ...
  it('should get the correct fee for a given pair and the amount', async () => {
    ...
    await starSwap.withdraw(erc20Token.getAddress(), ethers.parseEther('0.01'));

    await erc20Token.approve(starSwap.getAddress(), ethers.parseEther('2'));
    await starSwap.createPublicOrder({
      makerSellToken: erc20Token.getAddress(),
    ...
    await wmaticToken
      .connect(user2)
      .approve(starSwap.getAddress(), ethers.parseEther('4'));
    await starSwap.connect(user2).fillPublicOrder(1, user2.getAddress());
  ...
  it('should have the same fee no matter which token is a base token in a pair', async () => {
    ...
    await wmaticToken.approve(starSwap.getAddress(), ethers.parseEther('2'));
    await starSwap.createPublicOrder({
    ...
    expect(await wmaticToken.balanceOf(starSwap.getAddress())).to.equal(
      ethers.parseEther('0.02'), // 1% of 2
    );
  });
});
