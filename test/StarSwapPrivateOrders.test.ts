...
describe('PrivateOrders', () => {
  ...
  it('Should not allow other users to accept a private order', async () => {
    const { starSwap, privateOrderExecutor, erc20Token, wmaticToken } =
      await loadFixture(prepareTestEnv);
    const [user1, user2] = await ethers.getSigners();
    ...
    ).to.be.revertedWithCustomError(starSwap, 'StarSwap__YouAreNotARecipient');
  });
  ...
  it('Should not allow to fill the same private order multiple times', async () => {
    ...
    await expect(
      ...
      privateOrderExecutor,
      'StarSwap__OrderHaveAlreadyBeenExecuted',
    );
  });

  it('should allow to invalidate a private order', async () => {
    const { starSwap, privateOrderExecutor, erc20Token, usdcToken } =
      await loadFixture(prepareTestEnv);
    const [user1, user2] = await ethers.getSigners();
    ...
    await usdcToken
      .connect(user2)
      .approve(starSwap.getAddress(), ethers.parseUnits('100', 6));

    await erc20Token
      .connect(user2)
      .transfer(starSwap.getAddress(), await erc20Token.balanceOf(user2.getAddress()));
    await usdcToken
      .connect(user1)
      .transfer(starSwap.getAddress(), await usdcToken.balanceOf(user1.getAddress()));

    await expect(
      privateOrderExecutor.connect(user2).invalidatePrivateOrder(order),
    ).to.be.revertedWithCustomError(starSwap, 'StarSwap__NotAnOwner');

    await privateOrderExecutor.invalidatePrivateOrder(order);

    await expect(
      privateOrderExecutor.connect(user2).fillPrivateOrder(order, orderHash, signature),
    ).to.be.revertedWithCustomError(starSwap, 'StarSwap__OrderHaveAlreadyBeenExecuted');
  });
});
